...
1. **Tier2 真事件脚本（本轮落地）**：`tests/real_event_harness.zig`（zig build real_event → zig-out/bin/real_event）
   + `tests/scripts/{macos,linux}_real_event.sh` + `windows_real_event.ps1` 已实现并纳入文档/测试配置；
   **在 macvm / linuxvm / windowsvm 上真跑验证**留待各 VM 回归执行（经 vm-regression 白名单，本机禁执行）。
2. **iOS/Android no-op 语义（Tier3）**：hosts/proxy 移动 no-op 已由注入单测锁定；真实设备 E2E 为集成级、远期再说。
